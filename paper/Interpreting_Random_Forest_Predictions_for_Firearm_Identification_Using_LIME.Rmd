...
if(!file.exists("../data/hamby224_lime_inputs.rds")) {
  ...
  hamby224_lime_inputs <- data.frame(bin_continuous = c(rep(TRUE, 10), rep(FALSE, 2)),
                                     quantile_bins = c(rep(TRUE, 5), rep(FALSE, 5), 
                                                       rep(TRUE, 2)),
                                     nbins = c(rep(2:6, 2), rep(4, 2)),
                                     use_density = c(rep(TRUE, 10), TRUE, FALSE)) %>%
    mutate(case = 1:length(bin_continuous)) %>%
  ...
  hamby224_lime_explain <- future_pmap(.l = as.list(hamby224_lime_inputs %>% 
                                                    select(-case)),
...
if(!file.exists("../data/hamby224_sensitivity_inputs.rds")) {
  ...
  sensitivity_inputs <- data.frame(bin_continuous = rep(TRUE, 100),
                                   quantile_bins = rep(c(TRUE, FALSE), each = 50),
                                   nbins = rep(2:6, each = 10),
                                   use_density = rep(TRUE, 100)) %>%
    mutate(case = 1:length(bin_continuous)) %>%
    select(case, bin_continuous, nbins, quantile_bins, use_density)
  ...
  sensitivity_outputs <- future_pmap(.l = as.list(sensitivity_inputs %>% select(-case)),
             .f = run_lime, # run_lime is one of my helper functions
             train = hamby173and252_train %>% select(rf_features),
             test = hamby224_test %>% arrange(case) %>% select(rf_features) %>% na.omit(),
             rfmodel = as_classifier(rtrees),
    ...
    mutate(rep = factor(rep(rep(1:10, each = dim(hamby224_test %>% na.omit())[1] * 3), 10))) %>%
  ...
  saveRDS(sensitivity_inputs, "../data/hamby224_sensitivity_inputs.rds")
  saveRDS(sensitivity_outputs, "../data/hamby224_sensitivity_outputs.rds")
  
} else {
  
  # Load in the sensitivity inputs and outputs
  sensitivity_inputs <- readRDS("../data/hamby224_sensitivity_inputs.rds")
  sensitivity_outputs <- readRDS("../data/hamby224_sensitivity_outputs.rds")

}

sensitivity_results <- sensitivity_outputs %>%
  mutate(diff = rfscore - model_prediction) %>%
  group_by(rep, bin_continuous, nbins, quantile_bins, use_density) %>%
  summarise(mse = sum((diff)^2) / length(diff))

sensitivity_results %>%
  group_by(bin_continuous, nbins, quantile_bins, use_density) %>%
  summarise(mean = mean(mse),
            sd = sd(mse))

sensitivity_results %>%
  ggplot(aes(x = factor(nbins), y = mse)) + 
  geom_boxplot() + 
  geom_point()
```
