...
if(!file.exists("../data/hamby224_lime_inputs.rds")) {
  ...
  hamby224_lime_inputs <- list(bin_continuous = c(rep(TRUE, 15), rep(FALSE, 2)),
                               quantile_bins = c(rep(TRUE, 5), rep(FALSE, 5), 
                                                 rep(TRUE, 5), rep(TRUE, 2)),
                               nbins = c(rep(2:6, 3), rep(4, 2)),
  ...
  names(hamby224_lime) <- map_chr(1:length(hamby224_lime_inputs), function(case) 
      sprintf("case: bin_continuous = %s, quantile_bins = %s, nbins = %0.f, use_density = %s, bin_method = %s",
...
if(!file.exists("../data/hamby224_test_explain.rds")) {
  ...
  hamby224_test_explain <- hamby224_test_explain %>%
    ...
    select(situation, bin_situation, bin_continuous:bin_method, case:feature_desc,
...
if(!file.exists("../data/hamby224_lime_comparisons.rds")) {
  ...
  hamby224_lime_comparisons <- hamby224_test_explain %>%
    ...
    slice(1) %>%
    ungroup() %>%
    select(situation, bin_situation, bin_method, bin_continuous, quantile_bins, nbins, use_density, 
