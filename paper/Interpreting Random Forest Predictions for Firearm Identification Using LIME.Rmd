...
if(!file.exists("../data/hamby224_lime.rds")) {
  ...
  hamby224_lime_cases <- data.frame(bin_continuous = rep(TRUE, 18),
                                    quantile_bins = c(rep(TRUE, 9), rep(FALSE, 9)),
                                    nbins = rep(2:10, 2),
                                    use_density = rep(NA, 18)) %>%
  ...
  hamby224_all_results <- pmap(.l = as.list(hamby224_lime_cases[1,-1]),
  ...
  names(hamby224_lime) <- map_chr(1:dim(hamby224_lime_cases[1,])[1], function(case) 
...
hamby224_lime_comparisons <- hamby224_test_explain %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(bin_continuous, quantile_bins, nbins, use_density, case, set, rf_features, rfscore, model_prediction, model_r2) %>%
...
ggplot(hamby224_lime_comparisons, aes(x = rfscore, y = model_prediction)) + 
  geom_point(aes(color = model_r2)) + 
  facet_grid(nbins ~ set) + 
  theme_bw() + 
  stat_smooth(method = "lm")
```

The plots below show the absolute value of the difference between the random forest probability and the simple model probability versus the random forest probability. These plots show a clear v-shaped trend that suggests that the simple model is doing the best job of replicating the random forest probability around the random forest probability of 0.5. That is, the simple model seems to be doing a worse job of computing accurate predictions in the extremes of the random forest scores.

```{r fig.width = 8, fig.height = 4, eval = FALSE}
ggplot(hamby224_lime_comparisons, aes(x = rfscore, y = abs(diff))) + 
  geom_point(aes(color = model_r2)) + 
  facet_grid(nbins ~ set) +
  theme_bw()
```
...
```{r fig.width = 8, fig.height = 4, eval = FALSE}
ggplot(hamby224_lime_comparisons, aes(x = rfscore, y = diff)) + 
  geom_point(aes(color = model_r2)) + 
  facet_grid(nbins ~ set) +
  geom_hline(yintercept = 0, color = "blue") +
  theme_bw()
