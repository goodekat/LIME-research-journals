...
if (!file.exists("../data/hamby_bins.csv")) {
  ...
  nbins <- length(hamby224_lime$bin_cuts[[1]]) - 1
  
  # Create a dataframe of bin boundaries
  hamby_divisions <- data.frame(Feature = rf_features, 
                                Lower = c(0, -1, 0, 0, 0, 0, 0, 0, 0),
                           matrix(unlist(hamby224_lime$bin_cuts), nrow = 9, 
                                  byrow = TRUE)[,2:nbins],
                           Upper = c(1, 1, rep(NA, 7)))
  
  # Use my function "write_bins" to create a dataframe with the bins
  hamby_bins <- data.frame(t(apply(hamby_divisions, 1, write_bins)))
  
  # Assign appropriate names to the bin columns
  if (nbins == 2){
    names(hamby_bins) <- c("Feature", "Lower Bin", "Upper Bin")
  } else {
    names(hamby_bins) <- c("Feature", "Lower Bin", 
                           sapply(1:(nbins-2), 
                                  function(bin) sprintf("Middle Bin %.0f", bin)),
                           "Upper Bin")
  }
...
check <- hamby224_test_explain %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(case, set, rf_features, rfscore, model_prediction, model_r2) %>%
