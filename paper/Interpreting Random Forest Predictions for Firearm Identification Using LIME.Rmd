...
```{r}
# Create a subsampled version of the training data
if (!file.exists("../data/hamby173and252_train_sub.csv")) {
  ...
  # Load in the subsampled training data
  hamby173and252_train_sub <- read.csv("../data/hamby173and252_train_sub.csv")
...
if(!file.exists("../data/hamby224_lime_inputs.rds")) {
  ...
  hamby224_lime_inputs <- data.frame(data_name = c(rep("Full", 12), rep("Subsampled", 12)),
                                     bin_continuous = rep(c(rep(TRUE, 10), rep(FALSE, 2)), 2),
                                     quantile_bins = rep(c(rep(TRUE, 5), rep(FALSE, 5), rep(TRUE, 2)), 2),
                                     nbins = rep(c(rep(2:6, 2), rep(4, 2)), 2),
                                     use_density = rep(c(rep(TRUE, 10), TRUE, FALSE)), 2) %>%
    mutate(case = 1:length(data_name)) %>%
    select(case, data_name, bin_continuous, nbins, quantile_bins, use_density)

  # Apply lime to the full training data with the specified input options
  hamby224_lime_explain_full <- pmap(.l = as.list(hamby224_lime_inputs %>% 
                                                    filter(data_name == "Full") %>%
  ...
  hamby224_lime_full <- map(hamby224_lime_explain_full, function(list) list$lime)
  hamby224_explain_full <- map_df(hamby224_lime_explain_full, function(list) list$explain)

  # Apply lime to the subsampled training data with the specified input options
  hamby224_lime_explain_sub <- pmap(.l = as.list(hamby224_lime_inputs %>% 
                                               filter(data_name == "Subsampled") %>%
                                               select(-case, -data_name)),
             .f = run_lime, # run_lime is one of my helper functions
             data_name = "Subsampled",
  ...
  names(hamby224_lime) <- map_chr(1:dim(hamby224_lime_inputs)[1], function(case) 
      sprintf("case: data = %s, bin_continuous = %s, quantile_bins = %s, nbins = %0.f, use_density = %s",
              hamby224_lime_inputs$data_name[case],
...
if(!file.exists("../data/hamby224_test_explain.rds")) {
  ...
  hamby224_test_explain <- hamby224_test_explain %>%
    ...
    select(training_data:use_density, case:feature_desc, feature_bin:strictly_less, 
           data, prediction)
...
if(!file.exists("../data/hamby224_lime_comparisons.rds")) {
  ...
  hamby224_lime_comparisons <- hamby224_test_explain %>%
    ...
    group_by(training_data, case, bin_continuous, quantile_bins, nbins, use_density) %>%
    slice(1) %>%
    ungroup() %>%
    select(training_data, bin_continuous, quantile_bins, nbins, use_density, 
...
hamby224_lime_results <- hamby224_lime_comparisons %>%
  ...
  group_by(training_data, bin_continuous, quantile_bins, nbins, use_density, set) %>%
  summarise(mse = (sum(diff)^2) / length(diff),
            ave_r2 = mean(model_r2)) %>%
  arrange(training_data, set)
```

```{r}
hamby224_lime_results %>%
  group_by(training_data, set) %>%
  arrange(mse) %>%
  slice(1:2)
```
...
hamby224_test_explain %>%
  filter(training_data == "Full", set == "Set 1", nbins == "2") %>%
