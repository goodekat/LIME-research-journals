...
#                                        type = "prob")[,2]
# 
# Save the adjusted training data
# write.csv(hamby173and252_train %>% select(-X), "../data/hamby173and252_train.csv",
#           row.names = FALSE)
```
...
if(!file.exists("../data/hamby224_lime_inputs.rds")) {
  ...
  names(hamby224_lime) <- map_chr(1:length(hamby224_lime_inputs), function(case) 
      sprintf("case: bin_continuous = %s, quantile_bins = %s, nbins = %0.f, 
              use_density = %s, bin_method = %s, response = %s",
