...
4. Issues with LIME and attempts at a solution
...
1. Create perturbations based on training data.
...
- 240 total sets created
    - 10 barrels in a set
    - 35 bullets in a set 
        - 20 known (2 from the same barrel)
...
```{r}
hamby173and252_train <- read.csv("./data/hamby173and252_train.csv") # 83,028 rows
```
...
- Want to know which were the driving variables for individual predictions
- Especially great when the model is wrong
...
#### Draw many samples from the estimated feature distributions to obtain the perturbations

- If the a binning estimation method is used, numeric features are converted to indicator variables.
...
- Options in lime
     - auto: forward selection if $m\le6$, highest weight otherwise (default)
...
$$\mbox{rf score}=\beta_0 + \beta_1 I[\mbox{ccf in same bin}] + \beta_2 I[\mbox{rough_cor in same bin}] +\cdots + \beta_9 I[\mbox{sum_peaks in same bin}]$$
...
$$\mbox{rf score}=\beta_0 + \beta_1 I[\mbox{ccf in same bin}] + \beta_2 I[\mbox{cms in same bin}] + \beta_3 I[\mbox{matches in same bin}]$$
...
- Largest $\beta_i$ is considered the most important
...
.center[<font size="4">Example where the random forest prediction is wrong. The explanations are from from 4 quantile bins.</font>]
...
.center[<font size="4">This is the same case with explanations are from from 3 equally spaced bins.</font>]
...
- Non-binning methods produce very different results

---

# Future Work

- Apply LIME to explain a logistic regression model
...
- Adjust and compare weighting methods and the features selection methods
