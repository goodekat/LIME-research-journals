...
- write draft of firearm examiner's paper with everything but the methods and results sections
- find an idea that leads to better LIME explanations
- once the better LIME binning method has been selected, write the methods and results sections
- make plans for the statistical paper

Goals for week 1:

- set up meeting time with Heike
- get tree method up and running
- start to work on evaluation of the results of the new method
- start looking into the weights
- work on introduction and data section of firearm examiners' paper

### Visualizations of Tree Based Bins Results

```{r}
# I need to fix the situations to new options 
hamby224_test_explain %>%
  mutate(situation = ifelse(bin_continuous == TRUE & quantile_bins == TRUE, 
                            sprintf("%.0f quantile bins", nbins),
                            ifelse(bin_continuous == TRUE & quantile_bins == FALSE, 
  ...
  droplevels() %>%
  na.omit() %>%
  select(situation, case, feature, feature_weight) %>%
  ggplot(aes(x = case, y = situation, fill = feature)) + 
  geom_tile()
```
