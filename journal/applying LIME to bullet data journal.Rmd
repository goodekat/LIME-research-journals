...
chosen_features %>%
  ...
  summarise(kappa = 
              irr::kappam.fleiss(matrix(c(bins2, bins3, bins4, bins5, bins6), ncol = 5))$value) %>%
...
chosen_features %>%
  ...
  labs(x = "Bin Situation", y = "Count", fill = "Number of Features") + 
  theme_bw() +
...
hamby224_sensitivity_mses %>%
  mutate(bins = ifelse(nbins == 4 & bin_situation == "other", "other", as.character(nbins))) %>%
  ggplot(aes(x = situation, y = mse)) + 
  geom_boxplot(aes(color = bin_situation)) +
  facet_grid(set ~ bins, scales = "free_x") + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom") + 
  labs(x = "Situation", y = "MSE", color = "") + 
  scale_color_brewer(palette = "RdYlBu")
...
hamby224_consistency %>%
  ...
  facet_grid(set ~ bins, scales = "free_x") + 
  labs(x = "Situation", y = "Mean Number of Features", color = "") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom") + 
  scale_color_brewer(palette = "RdYlBu")
```
...
hamby224_sensitivity_results %>%
  ...
  summarise(nlevels = length(levels(factor(feature)))) %>%
  mutate(nlevels = factor(nlevels, levels = c(1:5))) %>%
  ggplot(aes(x = situation, fill = nlevels)) + 
  geom_bar(position = "stack") +
  facet_grid( ~ set) + 
  theme(legend.position = "bottom", 
        axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_fill_brewer(palette = "RdYlBu") + 
  labs(x = "Situation", y = "Count", fill = "Number of Features")
