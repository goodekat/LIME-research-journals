...
```{r}
# Creates a dataset that identifies the top chosen features by lim in order of first to third
...
In order to access consistency, I have decided to compute Fleiss' kappa for each of the bin types. Fleiss' kappa is often used to assess the consistency across raters for any number of raters and with nominal ratings. It calculates the degree of agreement in classification over that which would be expected by chance. 

Need to write how it is computed, so I can better understand it's use: https://en.wikipedia.org/wiki/Fleiss%27_kappa

I computed kappa for each bin situation separately for the top three features chosen. These values are plotted below for each set. For both the first and second features chosen by lime, the values of kappa suggest that quantile bins are the most consistent across all the nubmer of bins followed by the tree based bins. The values of kappa for the quantile bins are slight to fair agreement. For the third chosen variable, the tree based bins have the highest value of kappa, but all the kappa values for the third chosen feature are low.
...
chosen_features %>%
  ...
  summarise(kappa = 
              kappam.fleiss(matrix(c(bins2, bins3, bins4, bins5, bins6), ncol = 5))$value) %>%
...
ggplot(chosen_features, aes(x = situation, y = case, fill = first)) + 
  geom_tile() + 
...
hamby224_sensitivity_results <- hamby224_sensitivity_outputs %>%
  mutate(diff = rfscore - model_prediction,
         ...
         situation = ifelse(bin_continuous == TRUE & bin_method == "quantile_bins", 
                            ...
                            ifelse(bin_continuous == TRUE & 
                                   bin_method == "equally_spaced",
                                   sprintf("%.0f equally spaced bins", nbins),
                                   ifelse(bin_continuous == TRUE & bin_method == "tree",
                                          sprintf("%.0f tree based bins", nbins),
                                          ifelse(bin_continuous == FALSE & 
                                                 use_density == TRUE, "kernel density",
  ...
  mutate(bin_situation = ifelse(bin_method == "quantile_bins" & bin_continuous == TRUE,
                                ...
                                ifelse(bin_method == "equally_spaced" & 
                                       bin_continuous == TRUE, "equally spaced bins", 
                                       ifelse(bin_method == "tree" & bin_continuous == TRUE,
                                              "tree based bins", "other")))) %>%
...
hamby224_sensitivity_results %>%
  ...
  mutate(order = rep(rep(1:length(levels(case)), each = 10), 17)) %>%
