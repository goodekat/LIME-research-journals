...
\item \kg{words to go back and make sure I am not using too often: ability, produce}
\end{itemize}

\section{Introduction}

\kg{I added in Heike's suggestions to the first paragraph and made some changes of my own. I divided the first paragraph into two parts and added a discusssion on reasons why interpretability is beneficial. Note that I also switched the two model uses from ``interpretation and prediction" to ``inference and prediction". Since a key part of the paper is about being able to interpret predictive models, I thought it might be good to distinguish between interpretation and inference. Heike, what do you think?}\\
\\
In the field of statistics, there are two main uses for models: \kg{inference} and prediction. Machine learning models are often used for the latter purpose. While these models have been proven to perform well in a wide range of problems, their predictive accuracy comes at the cost of interpretability. They are often complex algorithms that are good at identifying patterns in the data but lack a functional form, so it not possible to directly interpret them. As a result, they produce predictions that lack an explanation, which has earned them the reputation of being black box models.

The ability to interpret a model serves multiple purposes. When a model is first fit, interpretations help to diagnosing the model. If it is possible to know what variables are influencing a prediction, it can be determined if the predictions are based on reasonable variables. If not, appropriate adjustments to the model can be made. After a model has been diagnosed and found to be reasonable, the explanations for the predictions can be used to understand the underlying mechanism that produced the data and provide an argument for why the predictions should be trusted. In some areas of application, this ability to be able to explain the results from a model is critical.
