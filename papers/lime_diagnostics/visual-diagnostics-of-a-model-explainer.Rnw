...
\item $x'\in\mathbb{R}^{p'}$: \hh{what is $p'$ ? shouldn't $p$ and $p'$ be the same? I thought that you were trying to say that $x'$ explains $x$?} vector for the interpretable representation of the instance being explained (e.g. a vector of indicator variables associated with the features chosen through feature selection indicating whether or not the observed value is in the bin created by LIME for the feature)
\item $G$: class of potentially \hh{why potentially?} interpretable models (e.g. linear models, decision trees, rule lists)
