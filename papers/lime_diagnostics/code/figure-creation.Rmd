...
lime_plots <- plot_grid(plot_feature1, plot_feature2, ncol = 1)
...
lime_plots_bad <- plot_grid(plot_feature1_bad, plot_feature2_bad, ncol = 1)
...
# Save the plot
ggsave(filename = "../figures/lime_figure_bad.png",
       plot = lime_figure_bad,
       height = 6, width = 4)
```
...
# Generate data
set.seed(20190624)
all_observations <- data.frame(case = factor("Other", levels = c("Other", "Prediction of Interest")),
...
```{r}
# Specify a prediction of interest
prediction_of_interest <- data.frame(case = factor("Prediction of Interest", 
                                                   levels = c("Other", "Prediction of Interest")),
                                     feature1 = 0.07,
                                     feature2 = 200,
                                     prediction = 0.05)
```
...
```{r}
lime_data <- bind_rows(all_observations, prediction_of_interest)
```
...
# Fit the interpretable explainer models
lime_explainer <- lm(prediction ~ feature1 + feature2, data = lime_data, weights = distance)
lime_explainer_bad <- lm(prediction ~ feature1 + feature2, data = lime_data, weights = distance_bad)
```
...
lime_data_gathered <- lime_data %>%
  gather(key = feature, value = value, 2:3) %>%
  gather(key = distance_case, value = distance_value, 3:4)
```

```{r}
ggplot(data = lime_data_gathered, 
       mapping = aes(x = value, 
                     y = prediction, 
                     size = distance_value,
                     shape = case)) + 
  geom_point(alpha = 0.5) + 
  geom_abline(data = lime_data_gathered %>% 
                filter(feature == "feature1", distance_case == "distance"),
              intercept = coef(lime_explainer)[[1]] +
                coef(lime_explainer)[[3]]*prediction_of_interest$feature2, 
              slope = coef(lime_explainer)[[2]], 
              size = 1) +
  #stat_smooth(data = lime_data_gathered %>% filter(feature == "feature2")) +
  facet_grid(distance_case ~ feature, scales = "free") + 
  theme_bw()
```
