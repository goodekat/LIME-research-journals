...
The version of the `lime` packge that is considered here is `r packageVersion("lime")` (version when I first wrote this section: 0.5.0). I am only considering the `data.frame` versions of the functions. 
...
if(!file.exists("../../../data/lime_functions/lime_v0.5.0.rds")) {
  saveRDS(object = getAnywhere(lime.data.frame),
          file = "../../../data/lime_functions/lime_v0.5.0.rds")
} else {
  lime_v0.5.0 <- readRDS("../../../data/lime_functions/lime_v0.5.0.rds")  
  if(assert_that(are_equal(lime_v0.5.0, getAnywhere(lime.data.frame)))){
    getAnywhere(lime.data.frame)
  } 
}
```
...
1. Check to make sure that the `explainer` is an object output from the `lime` function. 
...
3. If the model type is regression, check to make sure both `labels` and `n_labels` are set to NULL. Otherwsie, return a warning saying that labels and n_labels are ignored with explaining regression models. Then set `n_lables` to 1 and `labels` to NULL.
...
7. Makes use of the [`permute_cases`](#permute_cases) function for data frames in the lime package. This is the function that creates the perturbations. One perturnabiton is created for each of the `n_permuations` specified.
...
if(!file.exists("../../../data/lime_functions/explain_v0.5.0.rds")) {
  saveRDS(object = getAnywhere(explain.data.frame),
          file = "../../../data/lime_functions/explain_v0.5.0.rds")
} else {
  explain_v0.5.0 <- readRDS("../../../data/lime_functions/explain_v0.5.0.rds")  
  if(assert_that(are_equal(explain_v0.5.0, getAnywhere(explain.data.frame)))){
    getAnywhere(explain.data.frame)
  } 
}
```
...
The *lime* package provides a variety of functions for extracting the model type from the complex model to be explained. These are shown below. Here is information from the package documentation about models supported by *lime*. 
...
```{r output type}
getAnywhere(output_type)
```

### `exp_kernel` helper {#exp_kernel}
...
### `permute_cases` helper {#permute_cases}
