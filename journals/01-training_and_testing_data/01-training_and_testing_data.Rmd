---
title: "01-training and testing data"
...
my_colors <- colors()[par_data %>% pull(category) %>% as.numeric() * 11]
table(my_colors)
```

The first plot shows the observations that were kept in aquamarine and the observations that were removed in bisque. These observations do not seem to be weird. I am wondering if I removed the correct points or not. The second plot shows the data plotted after the observations have been removed. There are not obvious points that really stand out as unusual, but there is so much data it also makes it hard to tell.

```{r}
MASS::parcoord(hamby173and252_train_cleaning %>% select(ccf:sum_peaks), col = my_colors)
MASS::parcoord(hamby173and252_train %>% select(ccf:sum_peaks))
```
