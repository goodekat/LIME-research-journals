...
This journal includes the code for applying lime to the random forest fit to the bullet data.
...
It applies LIME using my function `run_lime` specify the different input values and save the output from the `lime` function and the `explain` function in two lists. The `lime` and `explain` outputs and the input settings are saved as .rds files.
...
This code combines the test data and the explanations.
...
This code creates a dataset with summaries of the explanations.
...
write.csv(hamby224_sensitivity_joined, "../data/hamby224_sensitivity_joined.csv",
          row.names = FALSE)
```
